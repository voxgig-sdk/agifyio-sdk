...
class GetAgeEntity
{
    ...
    {
        $opts = $this->_entopts;
        return new GetAgeEntity($this->_client, $opts);
    }

    /**
     * @param GetAge|array $args GetAge data (assoc-array) to store.
     */
    public function data_set($args): void
    {
        if ($args) {
            $this->_data = AgifyioHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetData");
        }
    }

    /**
     * @return GetAge|array The current GetAge data as an assoc-array.
     */
    ...
    /**
     * @param array $args Match filter (any subset of GetAge fields).
     */
    public function match_set($args): void
    {
        if ($args) {
            $this->_match = AgifyioHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetMatch");
        }
    }

    /**
     * @return array The current match filter (any subset of GetAge fields).
     */
     ...
     *
     * @param GetAgeLoadMatch|array|null $reqmatch Match criteria (id/query
     *   fields) as an assoc-array; a typed GetAgeLoadMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return GetAge|array The loaded GetAge as an assoc-array at the
     *   SDK boundary; throws AgifyioError on failure (item-5 convention).
     */
        ...
        return $this->_run_op($ctx, function () use ($ctx) {
            if ($ctx->result) {
                ...
                if ($ctx->result->resdata) {
                    $this->_data = AgifyioHelpers::to_map(Struct::clone($ctx->result->resdata)) ?? [];
                }
            }
        });
    }
