...
class AgifyioSDK
{
    ...
    public function __construct(array $options = [])
    {
        ...
        $feature_opts = AgifyioHelpers::to_map(Struct::getprop($this->options, "feature"));
        if ($feature_opts) {
            $featureorder = Struct::getpath($this->options, "__derived__.featureorder");
            if (is_array($featureorder)) {
                foreach ($featureorder as $fname) {
                    $fopts = AgifyioHelpers::to_map($feature_opts[$fname] ?? null);
                    if ($fopts && isset($fopts["active"]) && $fopts["active"] === true) {
                        ...
                        if (!AgifyioFeatures::has_feature($fname)) {
                        ...
                        ($utility->feature_add)($this->_rootctx, AgifyioFeatures::make_feature($fname));
                    }
                }
            }
        }
    ...
    {
        return AgifyioUtility::copy($this->_utility);
    }
    ...
    public function prepare(array $fetchargs = []): mixed
    {
        ...
        $ctrl = AgifyioHelpers::to_map(Struct::getprop($fetchargs, "ctrl")) ?? [];
        ...
        $params = AgifyioHelpers::to_map(Struct::getprop($fetchargs, "params")) ?? [];
        $query = AgifyioHelpers::to_map(Struct::getprop($fetchargs, "query")) ?? [];
    ...
    private function op_denied(string $op): array
    {
        ...
        return [
            "ok" => false,
            "err" => new AgifyioError($op . "_allow",
                "AgifyioSDK: " . $op . ": operation not allowed by" .
    ...
    private function raw_request(array $fetchargs = []): mixed
    {
        ...
        if (is_array($fetched)) {
            $status = AgifyioHelpers::to_int(Struct::getprop($fetched, "status"));
    ...
    public function graphql(string $query, ?array $variables = null, ?array $ctrl = null): mixed
    {
        ...
        if (is_array($errors) && 0 < count($errors)) {
            ...
            $res["ok"] = false;
            $res["err"] = new AgifyioError("graphql_error",
                "AgifyioSDK: graphql: " . $msg);
            $res["graphql"] = $errors;
        }
    ...
    public function GetAge($data = null)
    {
        ...
        if ($data === null) {
            if ($this->_get_age === null) {
                $this->_get_age = new GetAgeEntity($this, null);
            }
            return $this->_get_age;
        }
        return new GetAgeEntity($this, $data);
    }
