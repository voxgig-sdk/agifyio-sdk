...
class FtCtrl extends AgifyioControl
{
...
class FtClient
{
    ...
    private AgifyioUtility $utility;

    public function __construct(AgifyioUtility $utility, array $options)
    {
    ...
    public function get_utility(): AgifyioUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (AgifyioContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (AgifyioContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = AgifyioConfig::make_feature($name);
    ...
    private static function buildUrl(AgifyioSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof AgifyioError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new AgifyioResponse($fetched);
        }
    ...
    private function fail(AgifyioContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (AgifyioContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
