...
//     (\`new AgifyioSDK(...)\` / \`AgifyioSDK.test(...)\` become
//     \`AgifyioSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const TEST_SEED = {"entity":{"get_age":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new AgifyioSDK(...)\` and any
// \`AgifyioSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+AgifyioSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/AgifyioSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
