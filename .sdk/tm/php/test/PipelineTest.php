...
{
    private static function utility(): AgifyioUtility
    {
        return new AgifyioUtility();
    }
    ...
    private static function ctx(array $over = []): AgifyioContext
    {
        $utility = $over['utility'] ?? self::utility();
        $client = $over['client'] ?? new PlClient(['base' => 'http://h']);
        $ctx = new AgifyioContext([
            'client' => $client,
            'utility' => $utility,
        ], null);
        $ctx->op = new AgifyioOperation(['name' => 'load', 'entity' => 'x']);
        foreach ($over as $k => $v) {
    ...
    {
        return ($err instanceof AgifyioError) ? $err->sdk_code : '';
    }
    ...
    public function test_make_point_rejects_a_disallowed_operation(): void
    {
        ...
        $ctx->op = new AgifyioOperation(['name' => 'nope', 'entity' => 'x']);
        [$point, $err] = AgifyioMakePoint::call($ctx);
    ...
    public function test_make_point_rejects_an_operation_with_no_endpoints(): void
    {
        ...
        $ctx->op = new AgifyioOperation(['name' => 'load', 'entity' => 'x', 'points' => []]);
        [$point, $err] = AgifyioMakePoint::call($ctx);
        $this->assertNull($point);
    ...
    public function test_make_point_returns_the_single_point(): void
    {
        ...
        $ctx = self::ctx(['options' => ['allow' => ['op' => 'load,list,create,update,remove']]]);
        $ctx->op = new AgifyioOperation(['name' => 'load', 'entity' => 'x', 'points' => [$point]]);
        [$got, $err] = AgifyioMakePoint::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new AgifyioSpec(['method' => 'GET']);
        $ctx = self::ctx();
        $ctx->out['spec'] = $preset;
        [$got, $err] = AgifyioMakeSpec::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_response_guards_missing_spec_response_result(): void
    {
        $ctx = self::ctx([
            'spec' => null,
            'response' => new AgifyioResponse([]),
            'result' => new AgifyioResult([]),
        ]);
        [, $err] = AgifyioMakeResponse::call($ctx);
        $this->assertSame('response_no_spec', self::code($err));

        $ctx = self::ctx([
            'spec' => new AgifyioSpec([]),
            'response' => null,
            'result' => new AgifyioResult([]),
        ]);
        [, $err] = AgifyioMakeResponse::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'spec' => new AgifyioSpec(['step' => 's']),
            'response' => new AgifyioResponse(self::resp(404, null, ['x-a' => '1'])),
            'result' => new AgifyioResult([]),
        ]);
        [, $err] = AgifyioMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_2xx_parses_the_body_and_marks_ok(): void
    {
        $ctx = self::ctx([
            'spec' => new AgifyioSpec(['step' => 's']),
            'response' => new AgifyioResponse(self::resp(200, ['v' => 1])),
            'result' => new AgifyioResult([]),
        ]);
        [, $err] = AgifyioMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_records_to_ctrl_explain_when_explain_is_on(): void
    {
        $ctx = self::ctx([
            'spec' => new AgifyioSpec(['step' => 's']),
            'response' => new AgifyioResponse(self::resp(200, ['v' => 2])),
            'result' => new AgifyioResult([]),
        ]);
        $ctx->ctrl->explain = ['on' => true];
        AgifyioMakeResponse::call($ctx);
    ...
    {
        $preset = new AgifyioResponse(self::resp(299));
        $ctx = self::ctx([
            'spec' => new AgifyioSpec([]),
            'response' => new AgifyioResponse([]),
            'result' => new AgifyioResult([]),
        ...
        [$got, $err] = AgifyioMakeResponse::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null, 'result' => new AgifyioResult([])]);
        [, $err] = AgifyioMakeResult::call($ctx);
        $this->assertSame('result_no_spec', self::code($err));

        $ctx = self::ctx(['spec' => new AgifyioSpec([]), 'result' => null]);
        [, $err] = AgifyioMakeResult::call($ctx);
    ...
    public function test_make_result_list_op_wraps_resdata_into_entity_instances(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new AgifyioSpec(['step' => 's']),
            'result' => new AgifyioResult(['ok' => true, 'resdata' => [['a' => 1], ['a' => 2]]]),
        ]);
        $ctx->op = new AgifyioOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = AgifyioMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_result_empty_list_yields_empty_resdata(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new AgifyioSpec(['step' => 's']),
            'result' => new AgifyioResult(['ok' => true, 'resdata' => []]),
        ]);
        $ctx->op = new AgifyioOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = AgifyioMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new AgifyioResult(['ok' => true]);
        $ctx = self::ctx([
            'spec' => new AgifyioSpec([]),
            'result' => new AgifyioResult([]),
        ]);
        $ctx->out['result'] = $preset;
        [$got, $err] = AgifyioMakeResult::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_request_guards_a_missing_spec(): void
    {
        ...
        [, $err] = AgifyioMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $boom = new AgifyioError('boom', 'boom');
        $utility->fetcher = function (AgifyioContext $_c, string $_u, array $_f) use ($boom): array {
            return [null, $boom];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new AgifyioSpec(['step' => 's', 'method' => 'GET', 'base' => 'http://h', 'parts' => ['a']]),
        ]);
        [$response, $err] = AgifyioMakeRequest::call($ctx);
        $this->assertNull($err);
    ...
    {
        $utility = self::utility();
        $utility->fetcher = function (AgifyioContext $_c, string $_u, array $_f): array {
            return [null, null];
        };
        $ctx = self::ctx([
            'utility' => $utility,
    ...
    public function test_make_request_a_normal_transport_response_is_wrapped(): void
    {
        ...
        [$response, $err] = AgifyioMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertInstanceOf(AgifyioResponse::class, $response);
    ...
    {
        $utility = self::utility();
        $utility->make_fetch_def = function (AgifyioContext $c): array {
            return [null, $c->make_error('fetchdef_boom', 'boom')];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new AgifyioSpec(['step' => 's', 'method' => 'GET']),
        ]);
        [$response, $err] = AgifyioMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertNotNull($response->err);
    ...
    {
        $preset = new AgifyioResponse(self::resp(201));
        $ctx = self::ctx(['spec' => new AgifyioSpec([])]);
        $ctx->out['request'] = $preset;
        [$got, $err] = AgifyioMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null]);
        [, $err] = AgifyioMakeFetchDef::call($ctx);
    ...
    public function test_make_fetch_def_serialises_body_and_inits_missing_result(): void
    {
        ...
        [$fetchdef, $err] = AgifyioMakeFetchDef::call($ctx);
        $this->assertNull($err);
    ...
    {
        $ctx = self::ctx(['result' => new AgifyioResult(['ok' => true, 'resdata' => 42])]);
        $this->assertSame(42, AgifyioDone::call($ctx));
    }

    public function test_done_raises_the_error_when_not_ok(): void
    {
        $ctx = self::ctx(['result' => new AgifyioResult(['ok' => false])]);
        $this->expectException(AgifyioError::class);
        AgifyioDone::call($ctx);
    }

    public function test_make_error_returns_resdata_when_ctrl_throw_is_false(): void
    {
        $ctx = self::ctx(['result' => new AgifyioResult(['ok' => false, 'resdata' => 'fallback'])]);
        $ctx->ctrl->throw_err = false;
        $this->assertSame('fallback', AgifyioMakeError::call($ctx, null));
    }
    ...
    public function test_make_error_records_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->throw_err = false;
        $ctx->ctrl->explain = ['on' => true];
        AgifyioMakeError::call($ctx, null);
    ...
    public function test_feature_add_appends_in_call_order(): void
    {
        ...
        $a = new AgifyioBaseFeature();
        $b = new AgifyioBaseFeature();
        AgifyioFeatureAdd::call($ctx, $a);
        AgifyioFeatureAdd::call($ctx, $b);
        $this->assertSame([$a, $b], $client->features);
    }

    private static function named_feature(string $name): AgifyioBaseFeature
    {
        $f = new AgifyioBaseFeature();
        $f->name = $name;
        return $f;
    }
    ...
    public function test_feature_add_ordering_before_after_replace(): void
    {
        ...
        AgifyioFeatureAdd::call($ctx, $miss);
    ...
    private static function auth_ctx(array $options, ?array $headers): AgifyioContext
    {
        $client = new PlClient($options);
        return self::ctx([
            'client' => $client,
            'spec' => $headers === null ? null : new AgifyioSpec(['headers' => $headers]),
        ]);
    }
    ...
    public function test_prepare_auth_guards_a_missing_spec(): void
    {
        ...
        [, $err] = AgifyioPrepareAuth::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'response' => new AgifyioResponse(['headers' => null]),
            'result' => new AgifyioResult([]),
        ]);
        AgifyioResultHeaders::call($ctx);
