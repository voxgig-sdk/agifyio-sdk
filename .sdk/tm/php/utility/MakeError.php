...
class AgifyioMakeError
{
    public static function call(?AgifyioContext $ctx, mixed $err): mixed
    {
        if ($ctx === null) {
            require_once __DIR__ . '/../core/Context.php';
            $ctx = new AgifyioContext([], null);
        }
        $op = $ctx->op ?? new AgifyioOperation([]);
        $opname = $op->name;
        if ($opname === '' || $opname === '_') {
            $opname = 'unknown operation';
        }

        $result = $ctx->result ?? new AgifyioResult([]);
        $result->ok = false;
        ...
        $errmsg = ($err instanceof AgifyioError) ? $err->msg : (string)$err;
        ...
        $sdk_err = new AgifyioError('', $msg, $ctx);
        $sdk_err->result = ($ctx->utility->clean)($ctx, $result);
        $sdk_err->spec = ($ctx->utility->clean)($ctx, $spec);
        if ($err instanceof AgifyioError) {
