...
local GetAgeEntity = {}
GetAgeEntity.__index = GetAgeEntity


function GetAgeEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, GetAgeEntity)
  self._name = "get_age"
  self._client = client
...
function GetAgeEntity:get_name()
  return self._name
end


function GetAgeEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return GetAgeEntity.new(self._client, opts)
end
...
function GetAgeEntity:mark_deleted()
  self._deleted = true
end


function GetAgeEntity:deleted()
  return true == self._deleted
end


function GetAgeEntity:data_set(args)
  if args ~= nil then
...
function GetAgeEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function GetAgeEntity:match_set(args)
  if args ~= nil then
...
function GetAgeEntity:match_get()
...
function GetAgeEntity:stream(action, args, callopts)
...
function GetAgeEntity:_run_op(ctx, post_done)
  local utility = self._utility
