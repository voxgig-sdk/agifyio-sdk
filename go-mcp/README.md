...
MCP server exposing the Agifyio SDK as tools, built on the
...
| Tool | Args | Returns |
|------|------|---------|
| `agifyio_list` | `entity`, optional `query` map | First page of records as JSON |
| `agifyio_load` | `entity`, `query` (e.g. `{id:N}`) | Single record as JSON |
...
After install, restart Claude Code; the `agifyio_list` and
`agifyio_load` tools become available in new sessions.
