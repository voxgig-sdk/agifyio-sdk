...
[MCP](https://modelcontextprotocol.io) server exposing the Agifyio SDK as
two agent tools — `agifyio_list` and `agifyio_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/agifyio-mcp)
make build

# 2. Provide credentials via the environment
export AGIFYIO_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user agifyio \
  -- /absolute/path/to/agifyio-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./agifyio-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// agifyio_list: first page of records
{ "entity": "get_age" }
{ "entity": "get_age", "query": { } }

// agifyio_load: one record by id
{ "entity": "get_age", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/agifyio-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user agifyio \
     -- "$PWD"/dist/*/agifyio-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `agifyio_list` and `agifyio_load` tools now appear
   in new sessions. Ask the agent to *"list get_age using agifyio"*
   and it calls `agifyio_list` with `{"entity":"get_age"}`.
...
```sh
export AGIFYIO_APIKEY=sk_live_xxx            # API key
export AGIFYIO_BASE=https://api.example.com  # optional: override the API base URL
```
...
| Tool | Args | Returns |
|------|------|---------|
| `agifyio_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `agifyio_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `AGIFYIO_APIKEY` | API key sent with every request. |
| `AGIFYIO_BASE` | Optional override of the API base URL. |
...
```sh
./agifyio-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"agifyio_load","arguments":{"entity":"get_age","query":{"id":1}}}}'
```
