...
import (
	...
	omni "github.com/voxgig-sdk/agifyio-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.AgifyioError); is {
				out := map[string]any{
					"name":    "AgifyioError",
...
func omniCtx(arg any, client *sdk.AgifyioSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.AgifyioSDK); is {
			client = live
