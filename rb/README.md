...
The Ruby SDK for the Agifyio API — an entity-oriented client using idiomatic Ruby conventions.

The SDK exposes the API as capitalised, semantic **Entities** — for example `client.GetAge` — with named operations (`load`) instead of raw URL paths and query strings. Working with resources and verbs keeps call sites self-describing and reduces cognitive load.
...
```ruby
client = AgifyioSDK.test(testopts, sdkopts)
```
...
raise a `AgifyioError` (a `StandardError` subclass), so wrap
...
```
rb/
├── Agifyio_sdk.rb       -- Main SDK module
...
The main module (`Agifyio_sdk`) exports the SDK class
