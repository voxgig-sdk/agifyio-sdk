...
module AgifyioUtilities
  MakeRequest = ->(ctx) {
    ...
    if fetch_err
      ...
      response = AgifyioResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = AgifyioResponse.new(fetched)
    else
