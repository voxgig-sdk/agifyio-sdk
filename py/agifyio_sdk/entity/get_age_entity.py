...
from agifyio_sdk.agifyio_types import (
    GetAge,
    GetAgeLoadMatch,
)
...
class GetAgeEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return GetAgeEntity(self._client, opts)
    ...
    def load(self, reqmatch=None, ctrl=None) -> GetAge:
        utility = self._utility
        # reqmatch is optional: an entity with no id-like key loads with no
        # match. Treat None as an empty match so client.GetAge().load()
        # works with no args.
        if reqmatch is None:
            reqmatch = {}
