...
class TestGetAgeDirect:

    def test_should_direct_load_get_age(self):
        setup = _get_age_direct_setup({"id": "direct01"})
        _skip, _reason = runner.is_control_skipped("direct", "direct-load-get_age", "live" if setup["live"] else "unit")
        if _skip:
...
def _get_age_direct_setup(mockres):
