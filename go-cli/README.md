...
boru-driven command-line client **and** interactive REPL for the Agifyio
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/agifyio-cli)
make build
...
./agifyio-cli load 1 get_age            # {id:1} shorthand
./agifyio-cli load '{id:1}' get_age       # explicit match map

# 5. Override the API base URL for a single call
AGIFYIO_BASE=https://api.example.com ./agifyio-cli load 1 get_age

# 6. No arguments -> interactive REPL
./agifyio-cli
agifyio> load 1 get_age
agifyio> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/agifyio-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/agifyio-cli load 1 get_age
   ```
...
```sh
./agifyio-cli load 1 get_age          # scalar shorthand for {id:1}
./agifyio-cli load '{id:1}' get_age     # explicit match map
```
...
Run with no arguments to open a REPL (prompt `agifyio>`). Each line is
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `get_age`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/agifyio-cli`. |
...
(`./agifyio-cli <expr>`) and interactively (the REPL), and expressions compose the
