// Command Agifyio-cli is a CLI + REPL for the Agifyio SDK,
...
const prompt = "agifyio"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "get_age"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      AGIFYIO_APIKEY (api key), AGIFYIO_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: AGIFYIO_APIKEY carries the API key and
	// AGIFYIO_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("AGIFYIO_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("AGIFYIO_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewAgifyioSDK(opts)
